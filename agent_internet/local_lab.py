...
class LocalDualCityLab:
    ...
    def lotus_addresses(self, city_id: str) -> dict[str, dict | None]:
        link = self.plane.registry.get_link_address(city_id)
        network = self.plane.registry.get_network_address(city_id)
        return {
            "link": None if link is None else asdict(link),
            "network": None if network is None else asdict(network),
        }
