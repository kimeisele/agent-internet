...
def build_lotus_capability_manifest(*, base_url: str | None = None) -> dict:
    ...
    capabilities = [
        ...
        _capability("operation_feed", "Read a chronological feed of persisted Lotus operation receipts with optional paging and filters for AI reconciliation loops.", "GET", "/v1/lotus/operations", "list_operation_feed", ["lotus.read"], "read_only_feed", ["limit?", "after_operation_id?", "action?", "operator_subject?"]),
    ...
    return {
        ...
        "parseability": {
            ...
            "operation_feed_item_fields": ["operation_id", "request_id", "action", "operator_subject", "status", "created_at", "last_replayed_at", "replay_count", "response_payload_keys", "receipt_lotus_action", "receipt_http_path"],
